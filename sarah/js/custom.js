...
imgWrap.addEventListener("mousemove", function (event) {
  let x = event.offsetX; // 마우스 x축 좌표값 저장
  const screenWidth = document.body.offsetWidth; // 현재 브라우저 가로 크기 저장
  const imgElm = document.querySelectorAll("section img"); // 이미지 태그 저장
  const percent = Math.floor((x / screenWidth) * 200); // 현재 가로크기를 200으로 나눈 커서가 위치하는 x축 좌표 인덱스

  for (let j = 0; j < imgElm.length; j++) {
    imgElm[j].style.display = "none"; // 반복문으로 접근한 모든 사진의 display값을 none으로 변경
  }
